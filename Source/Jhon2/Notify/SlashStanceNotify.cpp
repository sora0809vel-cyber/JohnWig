...
#include "SlashStanceNotify.h"
#include "AbilitySystemBlueprintLibrary.h"
#include "AbilitySystemComponent.h"
#include "Abilities/GameplayAbilityTypes.h"

void USlashStanceNotify::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* NowAnimation, const FAnimNotifyEventReference& EventReference)
{
	...
	if (MeshComp)
	{
		...
		if (AActor* OwnerActor = MeshComp->GetOwner())
		{
			...
			if (UAbilitySystemComponent* ASComponent = UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(OwnerActor))
			{
				...
				//停止のタグを飛ばす
				ASComponent->HandleGameplayEvent(StanceFree, &FreezeEventData);
			}
		}
	}
}
